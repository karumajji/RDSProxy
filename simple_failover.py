"""
Amazon RDS MySQL
Simple RDS Instance failover monitoring script, leveraging only the DNS endpoints. This scripts
connects to the database using the RDS endpoint, approx. once a second and checks the role of
the database engine. If it cannot connect or it connects to a standby instead, it counts the elapsed time.

Changelog:
2019-10-28 - Initial release
2020-06-08 - Changed the SELECT Query to look into RDS MySQL Instance
